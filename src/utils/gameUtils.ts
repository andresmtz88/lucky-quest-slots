...
export function getXpNeededForNextLevel(level: number) {
  return level * 100;
}

export function getRandomPirateMoonSymbol() {
  const randomIndex = Math.floor(Math.random() * PIRATE_MOON_SYMBOLS.length);
  return PIRATE_MOON_SYMBOLS[randomIndex];
}

export function createRandomPirateMoonReels() {
  const newReels: string[][] = [];
  ...
  for (let reelIndex = 0; reelIndex < 5; reelIndex++) {
    ...
    for (let rowIndex = 0; rowIndex < 3; rowIndex++) {
      reel.push(getRandomPirateMoonSymbol());
    }
