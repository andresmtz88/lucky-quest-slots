...
export default function HomeScreen() {
  ...
  const [pirateMoonHistory, setPirateMoonHistory] = useState<string[]>([]);
    ...
    async function loadGameSave() {
      try {
        ...
        if (savedData !== null) {
          ...
          setPirateMoonHistory(parsedSave.pirateMoonHistory ?? []);
  ...
  if (screen === "pirateMoon") {
    return (
      <PirateMoonScreen
        ...
        pirateMoonHistory={pirateMoonHistory}
        setPirateMoonHistory={setPirateMoonHistory}
        onAwardXp={awardXp}
        onSpin={() => setSpinCount((currentCount) => currentCount + 1)}
        onWin={() => setWinCount((currentCount) => currentCount + 1)}
        onBack={() => setScreen("lobby")}
...
function PirateMoonScreen({
  ...
  pirateMoonHistory: string[];
  setPirateMoonHistory: Dispatch<SetStateAction<string[]>>;
  ...
  const [showPirateHistory, setShowPirateHistory] = useState(false);
  ...
  function addPirateHistoryEntry(entry: string) {
    setPirateMoonHistory((currentHistory) =>
      [entry, ...currentHistory].slice(0, 5)
    );
  }
  ...
  function handleSpin() {
    ...
    setTimeout(() => {
      ...
      const pirateMoonMessage = pirateMoonTriggered
        ? ` · Moon Treasure +${PIRATE_MOON_REWARD.toLocaleString()}`
        : "";
      ...
      if (winAmount > 0) {
        ...
        addPirateHistoryEntry(
          `Treasure Win +${winAmount.toLocaleString()}${pirateMoonMessage}`
        );

        const xpMessage = onAwardXp(earnedXp);

        setResultMessage(
          `Treasure Win +${winAmount.toLocaleString()} · +${earnedXp} XP${xpMessage}${pirateMoonMessage}`
        );
      } else {
        addPirateHistoryEntry(`No treasure${pirateMoonMessage}`);

        const xpMessage = onAwardXp(earnedXp);

        setResultMessage(`No treasure · +${earnedXp} XP${xpMessage}${pirateMoonMessage}`);
      }
      ...
      >
        ...
        <View style={styles.pirateMeterBox}>
          <View style={styles.jackpotHeaderRow}>
            <Text style={styles.pirateMeterTitle}>🌙 Moon Treasure Meter</Text>
          ...
          <Text style={styles.jackpotText}>
            Fill through spins to unlock a virtual treasure bonus.
          </Text>
        </View>

        <View style={styles.piratePreviewBox}>
          <Text style={styles.piratePreviewTitle}>🏴‍☠️ Sail for moonlit treasure</Text>

          <Text style={styles.piratePreviewText}>
            Match 3+ symbols on the glowing middle row to win virtual coins.
          </Text>
        </View>
        ...
        <View style={styles.spinControlPanel}>
          ...
          onPress={() => setShowPirateHistory(!showPirateHistory)}
        >
          <Text style={styles.collapsibleHeaderText}>
            {showPirateHistory ? "Hide Recent Spins" : "Show Recent Spins"}
          </Text>
        ...
        {showPirateHistory && (
          <View style={styles.historyBox}>
            <Text style={styles.historyTitle}>Pirate Moon Recent Spins</Text>

            {pirateMoonHistory.length === 0 ? (
              <Text style={styles.historyEmptyText}>No spins yet.</Text>
            ) : (
              pirateMoonHistory.map((entry, index) => (
