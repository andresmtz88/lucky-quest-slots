...
function NeonDragonScreen({
  ...
  function handleSpin(useFreeSpin = false) {
    ...
    setTimeout(() => {
      ...
      const newCoinBalance = coins - coinCost + winAmount + shieldRefund;
      ...
      if (winAmount > 0) {
        ...
        setResultMessage(
          `Payline win! You won ${winAmount.toLocaleString()} coins and earned ${earnedXp} XP.${xpMessage}`
        );
      } else {
        const xpMessage = onAwardXp(earnedXp);
        ...
        if (shieldRefund > 0) {
          setResultMessage(
            `No win this spin. Loss Shield refunded ${shieldRefund.toLocaleString()} coins. You earned ${earnedXp} XP.${xpMessage}`
          );
        } else {
          setResultMessage(`No win this spin. You earned ${earnedXp} XP.${xpMessage}`);
        }
      }
