...
function calculateMiddleRowWin(newReels: string[][]) {
  ...
  if (highestMatchCount >= 5) {
    return DEFAULT_BET * 50;
  }

  if (highestMatchCount === 4) {
    return DEFAULT_BET * 15;
  }

  if (highestMatchCount === 3) {
    return DEFAULT_BET * 5;
  }
...
function NeonDragonScreen({
  ...
  function handleSpin() {
    if (coins < DEFAULT_BET) {
      setResultMessage("Not enough coins. Claim a reward or complete objectives.");
      setLastWin(0);
      return;
    }

    const newReels = createRandomReels();
    const winAmount = calculateMiddleRowWin(newReels);
    const newCoinBalance = coins - DEFAULT_BET + winAmount;
  ...
  return (
    <SafeAreaView style={styles.slotContainer}>
      ...
      <View style={styles.betBox}>
        <Text style={styles.betText}>Bet: {DEFAULT_BET} coins</Text>
